...
describe('InMemoryRepository unit tests', () => {
  ...
  describe('applyFilter method', () => {
    it('should no filter items when filter param is null', async () => {
      ...
      const spyFilterMethod = jest.spyOn(items, 'filter'); // vigiar se algo foi chamado, no caso, filter.
  ...
  describe('applySort method', () => {
    it('should no sort items', async () => {
      const items = [
        ...
        new StubEntity({ name: 'a', price: 55 }),
      ];
  ...
  describe('applyPaginate method', () => {
    it('should paginate items', async () => {
      ...
      itemsPaginated = await sut['applyPaginate'](items, 1, 5);
      expect(itemsPaginated).toStrictEqual(items);
    });
  });
