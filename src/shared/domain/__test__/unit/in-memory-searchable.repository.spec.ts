...
describe('InMemoryRepository unit tests', () => {
  ...
  describe('applyFilter method', () => {
    it('should no filter items when filter para is null', async () => {
      const items = [new StubEntity({ name: 'name', price: 50 })];
    ...
    it('should filter using filter param', async () => {
      ...
      expect(itemsFiltered).toStrictEqual([items[0], items[1]]);
      expect(spyFilterMethod).toHaveBeenCalledTimes(2); // 2 vezes por estar dentro do mesmo teste

      itemsFiltered = await sut['applyFilter'](items, 'no-filter');
      expect(itemsFiltered).toHaveLength(0);
      expect(spyFilterMethod).toHaveBeenCalledTimes(3); // 3 vezes por estar dentro do mesmo teste
    });
  });
  ...
  describe('search method', () => {
    ...
    it('should search using paginate, filter and sort', async () => {
      const items = [
        new StubEntity({ name: 'test', price: 50 }), //0
        new StubEntity({ name: 'a', price: 50 }), //1
        new StubEntity({ name: 'TEST', price: 50 }), //2
        new StubEntity({ name: 'e', price: 50 }), //3
        new StubEntity({ name: 'TeST', price: 50 }), //4
      ];
      sut.items = items;
